...
public readonly record struct NsColor
{
    ...
    public static NsColor FromString(string colorString)
    {
        ...
        return Enum.TryParse(colorString, true, out BuiltInConstant enumValue)
            ? FromConstant(enumValue)
            : throw ThrowHelper.ArgumentInvalid(nameof(colorString));
    }
}
