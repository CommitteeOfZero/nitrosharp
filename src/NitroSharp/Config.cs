...
{
    public PhysicalSizeU RenderResolution { get; private init; }
    ...
    public static Config Read(Stream stream)
    {
        ...
        uint width = property("window.width").GetUInt32();
        uint height = property("window.height").GetUInt32();

        return new Config
        {
            RenderResolution = new PhysicalSizeU(width, height),
            ...
            EnableFullScreen = property("graphics.fullscreen").GetBoolean(),
