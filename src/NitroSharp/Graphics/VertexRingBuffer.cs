...
{
    internal sealed class VertexRingBuffer<TVertex> : VertexBuffer
        where TVertex : unmanaged, IEquatable<TVertex>
    {
        ...
        public VertexRingBuffer(GraphicsDevice graphicsDevice, uint initialCapacity)
        {
            _gd = graphicsDevice;
        ...
        {
            uint totalItems = _cursor;
            if (totalItems == 0) { return; }

            if (_firstUpdate || _changes.Count == totalItems)
            {
                uint size = _vertexSize * totalItems;
        ...
        private void Resize(uint itemCount)
        {
            ...
            uint size = MathUtil.RoundUp(itemCount * _vertexSize, 16);
            ...
            Array.Resize(ref _vertices, (int)itemCount);
        }
