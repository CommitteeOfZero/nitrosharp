...
{
    private static ReadOnlySpan<byte> Magic => "AFS\0"u8;
    ...
    {
        uint? fileId = null;
        if (_iniFileNames.TryGetValue(path, out uint iniFileName))
        {
            fileId = iniFileName;
        }
        else if (_builtinFileNames.TryGetValue(path, out uint builtinFileName))
        {
            fileId = builtinFileName;
        }
