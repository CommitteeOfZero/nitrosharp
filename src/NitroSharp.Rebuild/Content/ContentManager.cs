...
internal sealed class ContentManager : IDisposable
{
    ...
    {
        VfsNode head = _root;
        string[] pathParts = mountPoint.MountName.ToLowerInvariant().Split("/");
        foreach (string part in pathParts)
        {
            head.Children ??= new Dictionary<string, VfsNode>();
            if (!head.Children.TryGetValue(part, out VfsNode? value))
            {
                value = new VfsNode();
                head.Children[part] = value;
            }
            head = value;
        }
    ...
    {
        string[] pathParts = path.ToLowerInvariant().Split("/");
        VfsNode head = _root;
        ...
        int depth;
        for (depth = 0; depth < pathParts.Length - 1; depth++)
        {
            if (head.Children is not null)
            {
                head = head.Children[pathParts[depth]];
            }
            else
            {
                break;
        ...
        string archivePath = string.Join("/", pathParts, depth, pathParts.Length - depth);
