global using TextLayout = NitroSharp.Text.TextLayout<NitroSharp.DesignPixel>;
using System;
...
{
    internal sealed class TextLayout<TUnit>
    {
            ...
            float rubyFontSizeMultiplier = 0.4f)
        {
            ...
            MaxBounds = new Size<TUnit>(mWidth, mHeight);
        ...
        public Rectangle<TUnit> BoundingBox => _boundingBox;

        public Size<TUnit> MaxBounds { get; }
        ...
        public void Clear()
        {
            ...
            _caret = Vector2.Zero;
            _lastLine = default;
            _boundingBox = Rectangle<TUnit>.FromLTRB(
        ...
        public void Append(GlyphRasterizer glyphRasterizer, ReadOnlySpan<TextRun> textRuns)
        {
            ...
            var context = new TextLayoutContext<TUnit>
            {
                GlyphRasterizer = glyphRasterizer,
                MaxBounds = MaxBounds,
        ...
        append:
            var lines = new LineEnumerable<TUnit>(context, textRuns, _caret.X);
        ...
        exit:
            ...
            if (_lastLine is { HardBreak: true, IsEmpty: false })
            {
                NewLine();
            }

            _boundingBox = Rectangle<TUnit>.FromLTRB(left, top, right, bottom);
        }
    ...
    internal readonly struct TextLayoutContext<TUnit>
    {
        public GlyphRasterizer GlyphRasterizer { get; init; }
        public Size<TUnit> MaxBounds { get; init; }
    ...
    internal readonly struct Word
    {
        ...
        public float ActualAscender { get; init; }
        public float ActualDescender { get; init; }
        public float RubyTextAscender { get; private init; }

        public int Length => GlyphSpan.End.Value - GlyphSpan.Start.Value;

        public Word Update(Range glyphSpan, float rubyTextAscender)
        {
            return this with
            {
                GlyphSpan = glyphSpan,
    ...
    internal ref struct LineEnumerable<TUnit>
    {
        private readonly TextLayoutContext<TUnit> _context;
        private readonly float _caretStartX;
        private WordEnumerator<TUnit> _words;
        private Word? _peekedWord;
        private bool _firstLine;

        public LineEnumerable(
            in TextLayoutContext<TUnit> context,
            ReadOnlySpan<TextRun> textRuns,
            float caretStartX)
        {
            _context = context;
            _caretStartX = caretStartX;
            _words = new WordEnumerator<TUnit>(context, textRuns);
            _peekedWord = null;
            _firstLine = true;
            Current = default;
        }
        ...
        public LineEnumerable<TUnit> GetEnumerator() => this;
        ...
        public bool MoveNext()
        {
            ...
            while (_peekedWord is not null || _words.MoveNext())
            {
    ...
    internal ref struct WordEnumerator<TUnit>
    {
        private readonly TextLayoutContext<TUnit> _context;
        private CharacterEnumerator<TUnit> _characters;
        private Character? _peekedChar;

        public WordEnumerator(in TextLayoutContext<TUnit> context, ReadOnlySpan<TextRun> textRuns)
        {
            _context = context;
            _characters = new CharacterEnumerator<TUnit>(context, textRuns);
            _peekedChar = null;
            Current = default;
        }
        ...
        public bool MoveNext()
        {
            ...
            VerticalMetrics maxVMetrics = new();
            bool notEmpty = false;
            while (_peekedChar is not null || _characters.MoveNext())
            {
                notEmpty = true;
                ...
                {
                    GlyphDimensions dimensions = c.Font.GetGlyphDimensions(c.GlyphIndex, c.FontSize);
                    var glyph = new TextRunGlyph
                    {
                        Kind = c.Kind,
                        TextRunIndex = c.TextRun,
                        GlyphIndex = c.GlyphIndex,
                        Position = new Vector2(caret + dimensions.Left, -dimensions.Top),
                        IsWhitespace = Rune.IsWhiteSpace(c.Scalar)
                    };
                    glyphBuffer.Add(glyph);
                    caret += dimensions.Advance;
                    ascender = MathF.Max(ascender, dimensions.Top);
                    descender = MathF.Max(descender, dimensions.Height - dimensions.Top - 1);
                    bbRight = glyph.Position.X + dimensions.Width;
                    if (float.IsPositiveInfinity(bbLeft))
                    {
                        bbLeft = dimensions.Left;
                    }
    ...
    internal ref struct CharacterEnumerator<TUnit>
    {
        ...
        public CharacterEnumerator(in TextLayoutContext<TUnit> context, ReadOnlySpan<TextRun> textRuns)
        {
            _context = context;
            _textRuns = textRuns;
