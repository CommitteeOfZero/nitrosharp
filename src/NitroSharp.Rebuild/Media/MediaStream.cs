...
namespace NitroSharp.Media
{
    ...
    internal sealed class MediaStream : IDisposable
    {
            ...
            AudioParameters outAudioParams)
        {
            ...
            if (audioSource is not null && (_audio = OpenStream(audioStreamId)) is not null)
            {
                var options = new PipeOptions(minimumSegmentSize: 16384);
                _audioPipe = new Pipe(options);
            }
            if (graphicsDevice is not null && (_video = OpenStream(videoStreamId)) is not null)
            {
        ...
        public YCbCrBufferReader VideoFrames
        {
            get
            {
                if (_videoBuffer is null)
                {
                    NoVideoStream();
                    return default;
                }

                return _videoBuffer.Reader;
            }
        }
        ...
        public void Start()
        {
            ...
            if (_audio is not null && _audioSource is not null)
            {
                ...
                _audioSource.Play(_audioPipe.Reader);
            }

            if (_video is not null && _videoBuffer is not null)
            {
