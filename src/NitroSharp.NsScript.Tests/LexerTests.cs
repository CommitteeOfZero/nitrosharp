...
public class LexerTests
{
    ...
    {
        var tokens = Enum.GetValues<SyntaxTokenKind>();
    ...
    public void TokenPair(SyntaxTokenKind t1Kind, string t1Text, SyntaxTokenKind t2Kind, string t2Text)
    {
        ...
        Assert.Equal(lexResult.GetText(tokens[0]).ToString(), t1Text, ignoreCase: true);
    ...
    public void Recognizes_Certain_Quoted_Keywords(SyntaxTokenKind keyword)
    {
        ...
        var lexResult = SyntaxToken.Lex(quotedText);
        SyntaxToken token = lexResult.SingleToken();
    ...
    private static bool RequireSeparator(SyntaxTokenKind kind1, SyntaxTokenKind kind2)
    {
        ...
        bool canFormCompoundPunctuation(SyntaxTokenKind kind)
        {
            switch (kind1)
            {
        ...
        if (canFormCompoundPunctuation(kind1) && canFormCompoundPunctuation(kind2))
        {
            return true;
        }
