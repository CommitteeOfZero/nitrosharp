using TextLayout = NitroSharp.Text.TextLayout<NitroSharp.DesignPixel>;
using System;
...
public class TextLayoutTests(FontContext context) : IClassFixture<FontContext>
{
    private readonly GlyphRasterizer _rasterizer = context.Rasterizer;
    private readonly FontFaceKey _font = context.DefaultFont;
...
Nullam nisl ipsum, semper ac lacus sit amet, semper viverra diam.";
    ...
    public void MultipleLines_AppendEach(string newline)
    {
        ...
        AssertLines(layout, ["A gaze", "falls from the sky."]);
    }
    ...
    public void AppendText_EndingWithNewline(string newline)
    {
        ...
        AssertLines(layout, ["A gaze", "fell from the sky."]);
    }
    ...
    public void AppendText_SameLine()
    {
        ...
        Rectangle<DesignPixel> bbBefore = layout.BoundingBox;
        TextRun second = Regular(" falls from the sky.");
        layout.Append(_rasterizer, second);
        Rectangle<DesignPixel> bbAfter = layout.BoundingBox;
        AssertLine(layout, layout.Lines[0], "A gaze falls from the sky.");
    ...
    public void RubyText(string rb, string rt)
    {
        ...
        TextLayout layout = Layout([run], null, null, 45);
